...
TEST_CASE(aseprite_first_frame_layers) {
    ...
    add(aseLayer(0, 0, 0, 8, 255, "Hidden hard light")); // 2: hidden, Hard Light (not in NekoPhoto)
    ...
    CHECK(!hard.visible);
    CHECK(hard.blendMode == BlendMode::Normal);
    CHECK_NEAR(hard.opacity, 51 / 255.0, 1e-9);
    CHECK(!hard.parentId);
    CHECK(!doc.layers[3].asset);
    CHECK_EQ(imported->notes.size(), size_t(2));   // the second frame, and Hard Light
