...
to paint it with `brush.stroke` `mask` and the other mask tools, `show` to see it; deselect, or selecting a layer, ends that). `layers.warp` bends a layer with one of Photoshop's fifteen presets (`style`, `bend`, `horizontal`, `vertical`,
