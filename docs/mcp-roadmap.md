...
| 2 | **Close the gaps between socket and bridge.** `tabs_new`, `tabs_close`, `layers_get`, `layers_flip`, `layers_reorder`, `canvas_flip`, `colors_set`, `tool_select`, `view_zoom`, `history_info`, `selection_all`, `selection_none`, `selection_invert` as first-class tools, so the generic `rpc` tool is never needed for ordinary work. | Tool descriptions are what the model reads; a method hidden behind `rpc` is one it will not find. | `mcp/nekophoto_mcp.py` | ¼ |
