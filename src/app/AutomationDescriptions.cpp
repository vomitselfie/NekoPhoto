...
const MethodDoc methodDocs[] = {
    ...
    {"document.open", "Open a project (.comp), Photoshop (.psd/.psb), Clip Studio (.clip), SVG (.svg/.svgz: shapes as editable vector shape layers, the rest as pixels), PDF (.pdf: one page rendered as a pixel layer, when app.info reports pdf), image or camera RAW file (when app.info reports raw). With a document already open, an image is imported as a layer.",
    ...
    {"document.export", "Export as a layered Photoshop .psd (the reply lists what Photoshop cannot carry), as .svg (vector shape layers as paths, folders as groups, other layers as embedded PNGs; the reply counts them and lists what became images), or the composite as .png, .jpg, .webp or .tif (the extension decides).",
