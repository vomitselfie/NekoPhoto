...
const MethodDoc methodDocs[] = {
    ...
    {"document.open", "Open a project (.comp), Photoshop (.psd/.psb), Clip Studio (.clip), Affinity (.afphoto/.afdesign/.afpub/.af), Aseprite (.ase/.aseprite, first frame), SVG (.svg/.svgz: shapes as editable vector shape layers, the rest as pixels), PDF (.pdf: one page as a pixel layer, when app.info reports pdf), icon (.ico/.cur, a layer per size), animated GIF (a layer per frame), image or camera RAW file (.tga included; RAW when app.info reports raw). Layered files open in a new tab and the reply lists their layers and notes; with a document already open, an image is imported as a layer.",
    ...
    {"document.export", "Export as a layered Photoshop .psd (the reply lists what Photoshop cannot carry), as .svg (vector shape layers as paths, folders as groups, other layers as embedded PNGs; the reply counts them and lists what became images), or the composite as .png, .jpg, .webp, .tif, .tga or .ico (16, 32, 48 and 256 px; the extension decides).",
